...
async def startup_notification_system():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"?? Loaded automation settings for {len(automation_settings)} users")
        except FileNotFoundError:
            automation_settings = {}
            logger.info("?? No existing automation settings found")
...
async def save_automation_settings(settings: AutomationSettings):
    ...
    try:
        ...
        logger.info(f"?? Automation settings saved for {settings.user_id}")
    ...
    try:
        ...
        if success:
            ...
            if user_id not in automation_settings:
                ...
                logger.info(f"?? Default automation settings created for {user_id}")
    ...
    try:
        ...
        if biometric_matcher and user_id in biometric_profiles:
            ...
            if user_id in automation_settings:
                ...
                logger.info(f"?? Automation settings removed for {user_id}")
