    ...
    try:
        ...
        def on_connect(client, userdata, flags, rc):
            if rc == 0:
                ...
                # Subscribe to MR60BHA2 topics from Home Assistant
                topics = [
            ...
            try:
                ...
                if "heart_rate/state" in topic:
                    try:
                        ...
                        if 40 <= heart_rate <= 180:  # Valid range
                            ...
                            # TODO: Trigger biometric matching here
                            # await process_biometric_authentication(heart_rate)
...
async def process_biometric_authentication(heart_rate: float):
    """
    Process heart rate data for biometric authentication
    
    Args:
        heart_rate: Heart rate value from MR60BHA2 sensor
    """
    try:
        # TODO: Integrate with existing biometric matcher
        # This would call your existing biometric matching logic
        # with real heart rate data instead of simulated data
        
        logger.info(f"🔍 Processing biometric authentication for HR: {heart_rate}")
        
        # Example integration point:
        # from backend.utils.biometric_matcher import match_user_profile
        # result = await match_user_profile(heart_rate_data=[heart_rate])
        # 
        # if result["confidence"] > 0.80:
        #     logger.info(f"✅ User identified: {result['user_id']}")
        #     # Trigger Ring-style notification
        #     await send_presence_notification(result)
        
    except Exception as e:
        logger.error(f"❌ Error in biometric authentication: {e}")

def get_recent_heart_rate_data(limit: int = 10) -> list:
    """Get recent heart rate readings"""
    return mr60bha2_status["heart_rate_buffer"][-limit:] if mr60bha2_status["heart_rate_buffer"] else []

def get_current_presence() -> bool:
    """Get current presence detection status"""
    return mr60bha2_status["presence_detected"]